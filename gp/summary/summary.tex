...
For a toy case where $D=1,M=1$, we see that the asymmetric MTGP described just now is able to utilize side information when they help Figure (\ref{fig:plt_mtgp_shift=0.1}) and ignore side information when they are irrelevant Figure (\ref{fig:plt_mtgp_shift=1.5}). A summary of model performance for main task regressor with varying side information relevance is in Table (\ref{table:amtgp_mse_wrt_shift}). A summary of how much \texttt{amtgp} depends on side information is in Table (\ref{table:amtgp_lengthscale_ratio})
...
\begin{center}
    ...
    \hline
        0.000 &  0.069 &  0.052 &   0.046 \\
        0.100 &  0.083 &  0.052 &   0.082 \\
        0.500 &  0.188 &  0.098 &   0.187 \\
        1.000 &  0.360 &  0.403 &   0.366 \\
        1.500 &  0.433 &  0.881 &   0.458 \\
    \end{tabular}
    \caption{Mean squared error for main task (task=1) regressor at test locations along the grid ($\texttt{linspace}$(0,1,100)) for the three different methods \texttt{stgp},\texttt{mtgp},\texttt{amgtp} trained on data of varying task similarity. Larger values of \texttt{shift} implies the two task functions are more unrelated. \texttt{mtgp} is really effective when the two functions are coorperative but fails in cases where the two functions are dissimilar. \texttt{amtgp} is able to utilize side information when they are really relevant $\texttt{shift}=0$, and does not suffer from performance drop when the two tasks are irrelevant $\texttt{shift}=1.5$ }
...
\begin{center} 
\begin{figure}[h!]
    \includegraphics[width=\textwidth]{{{assets/plt_mtgp_shift=1.5}}}
    \caption{(Top) \texttt{stgp} GP regressors are learnt independently (Middle) \texttt{mtgp} GP regressors learnt via MTGP (Bottom) \texttt{amtgp} GP regressor for task 1 (main task) where the GP prior has kernel defined in Equation (\ref{eq:amtgp_product_kernel}, \ref{eq:amtgp_ard_kernel}). Missing side information is imputed from the predictive mean of a pre-trained \texttt{stgp} GP regressor for task 0. Bandwidth for the squared exponential kernel $k_x$ and ARD kernel $k_s$, e.g. $\pc{\ell_x, \ell_s}$, is optimized by maximizing the marginal likelihood of task 1 data points. We see, in case where the tasks are unrelated and perhaps even conflicting, as measured by a large $\texttt{shift}=1.5$ betwen these two task functions, we see \texttt{mtgp} sacrifice the performance of main task while \texttt{amtgp} learns to ignore side information as seen from large values of $\ell_s$, and therefore maintains its performance as measured by MSE along the grid points. }
    \label{fig:plt_mtgp_shift=1.5}
\end{figure}
\end{center} 
