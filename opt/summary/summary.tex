...
\begin{align*}
    g_w
        &= w - C\sum_{i=[m]: y_i(w^Tx_i + b) < 0 } y_i x_i \\
    g_b
        &= C\sum_{i=[m]: y_i(w^Tx_i + b) < 0 } y_i
\end{align*}
where we haved picked $0\in\partial (\max{0, 1- y_i(w^Tx_i + b)})$ when $y_i(w^Tx_i + b) = 1$, the only case where the \textit{max term} is non-differentiable.
