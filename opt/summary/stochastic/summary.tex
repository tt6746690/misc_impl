...
\end{align}
where the expectation is taken w.r.t $\xi_{[k-1]}$. We first derive a bound on $R_k^2 = \norm{x^k-x^*}_2^2$ and $r_k^2 = \E\pb{R_k}$,
\begin{align*}
    R_{k+1}^2
        &= \norm{x^k-x^*}^2 \\
...
\begin{align*}
    ...
    2\alpha_k \E\pb{ f(x^k) - f(x^*) }
        ...
        \leq r_k^2 - r_{k+1}^2 + \alpha_k^2 M^2
            \tag{Rearrange, and by \ref{main-eq:definition_convex}}
\end{align*}
Telescape and rearrange,
\begin{align*}
...
\end{align*}
Let $\tilde{x}^k = \sum_{i=1}^k \gamma_i x^i$. $f(\tilde{x}^k)\leq \sum_i \gamma_i f(x^i)$ by convexity of $f$. $\sum_i\gamma_i = 1$, we have 
\begin{align*}
    \E\pb{f(\tilde{x}^k) - f(x^*)}
        &\leq \frac{r_1 + M^2 \sum_{i=1}^k \alpha_i^2}{2\sum_{i=1}^k \alpha_i}
\end{align*}
