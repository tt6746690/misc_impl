...
\end{align*}
where we have used fact $2\cos(a)\cos(b)=\cos(a+b)+\cos(a-b)$ and that $e^{i\omega^Tx} = \cos(\omega^Tx) - i\sin(\omega^Tx)=\cos(\omega^Tx)$ when $p(\omega)$ is real-valued. We can estimate $k(\delta)$ via Monte Carlo estimates. Now define random Fourier feature map $z:\R^d\to\R^L$ as
\begin{align}
    z(x) 
...
\begin{align}
    z(x)^Tz(y)
        ...
        = k(x-y)
\end{align}
where convergence is exponential via Hoeffding's Inequality using the fact that $z_{\omega}(x)z_{\omega}(y) \in [-2,2]$,
\begin{align}
    \bbP\pb{ z(x)^Tz(y) - k(x-y) > \epsilon }
        \leq e^{-\frac{L\epsilon^2}{8}}
\end{align}
