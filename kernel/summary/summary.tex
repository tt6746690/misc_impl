...
Computing the kernel matrix is $\sO(n^2)$ while inversion requires $\sO(n^3)$, infeasible for large $n$. 
\begin{theorem}
    (Bochner's theorem) A bounded continous shift-invariant kernel $k(x,x') = k(x-x')$ on $\R^d$ is positive definite if and only if there exists a finite non-negative Borel measure $\Lambda$ on $\R^d$ such that 
    \begin{align}
        k(x-x')
...
We can think of $\Lambda$ as the fourier transform of $k$. If kernel is normalized such that $k(0)=1$, then $\Lambda$ will be a probability measure and $k = \E\pb{ e^{i\inner{\omega}{\cdot}} }$ corresponds to its characteristic function. If $k$ is Gaussian rbf kernel $k(x,x') = \exp(-\frac{1}{2\sigma^2}\norm{x-x'}^2)$, then $\Lambda \sim \sN(0,\frac{1}{\sigma^2}I)$. 

Bochner theorem can be applied to approximate kernel matrix. \cite{rahimiRandomFeaturesLargeScale2007} approximates the integral (\ref{eq:bochner}) with Monte Carlo estimates. The idea is to map data to a low-dimensional Euclidean space using a randomized feature map $z:\R^d \to \R^D$ where $D\ll N$ so that $k(x,y) = \inner{\varphi(x)}{\varphi(y)}_{\sH} \approx z(x)^Tz(y)$. 
...
\begin{definition}
    (Universal kernels) A continuous positive definite kernel on compact metric space $\sX$ is universal if the corresponding RKHS $\sH$ is dense in $C(\sX)$, space of bounded continous functions over $\sX$.
\end{definition}
