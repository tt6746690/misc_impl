...
Intuitively, we can compare dependence between two random variables by computing the distance in a RKHS. \cite{grettonMeasuringStatisticalDependence2005} first proposes HSIC as Hilbert-Schmidt norm of covariance operator, \cite{muandetKernelMeanEmbedding2017} provides a good summary on the topic. \cite{grettonKernelMethodsMeasuring2005} provides good summary of several related kernel dependency measures like COCO and KMI. Given separable RKHS $\sH,\sG$ with reproducing kernels $k,l$ over $\sX \times \sY$. Define the joint kernel $v:\sX\times\sY\to\R$ as a product kernel $v((x,y),(x',y') = k(x,x')l(y,y')$. HSIC is simply MMD between kernel embedding of joint $\bbP_{XY}$ with that of product of marginals $\bbP_X\bbP_Y$ over RKHS $\sH\times\sG$ \cite{grettonKernelMethodsMeasuring2005},
\begin{align}
    \text{HSIC}(X,Y)
        &= \norm{\mu_{\bbP_{XY}} - \mu_{\bbP_X \bbP_Y}}_{\sH\times\sG}
\end{align} 
whose computation can be delegated to kernels again,
\begin{align}
    \text{HSIC}(X,Y)
...
\end{align}
where we have a simple biased estimate,
\begin{align}
    \widehat{\text{HSIC}}_k(X,Y)
        = \frac{1}{m^2} \tr(\widetilde{\bK}\widetilde{\bL})
\end{align}
where $\widetilde{\bK},\widetilde{\bL}$ are centered kernel matrices computed from kernels $k,l$ and $\bH = \bI-\frac{1}{m} \mathbf{1}\mathbf{1}^T$ is the centering matrix (Refer to example (\ref{exampe_centering_matrix}) for centering in feature space). Since $\bH\bH = \bH$, we have,
\begin{align}
...
\begin{example}
    ...
    \begin{align}
        \widetilde{\bK}_{ij}
            ...
            &= (\bH\bK\bH)_{ij}
    \end{align}
    where $\mathbf{1}_n \in \R^{n\times n}$ defined as $(\mathbf{1}_n)_{ij}=\frac{1}{n}$. Therefore, $\widetilde{\bK} = \bH \bK \bH$ where $\bH = \bI - \mathbf{1}_n$.
\end{example}
...
\begin{center} 
    ...
    \caption{Use stochastic gradient descent to optimize for (\ref{eq:hsic_2d_gaussian_optimization}) where we have used RBF kernel with bandwidth chosen using median heuristic. We plotted transformed point by the linear mappings and its estimated density using Gaussian KDE. We also plotted contours of density of $Y$. Note as optimization progresses, the transformed points look more and only like an isotropic Gaussian as desired.}
