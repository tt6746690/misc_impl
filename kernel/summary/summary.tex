...
\begin{center} 
    ...
    \caption{The unnormalized witness function $\hat{f^*} = \widehat{\mu_{\bbP}} - \widehat{\mu_{\bbQ}}$ where $\text{N}(0,1)$ is compared with $\text{Lap}(0,\sqrt{.5})$. Both distribution has same mean and variance. The witness is computed from $4\times 10^4$ samples, using a Gaussian kernel with $\sigma^2=0.5$. This plot reproduces one figure in \cite{grettonKernelTwoSampleTest2012}}
