...
% can be approximated via Monte Carlo samples, naively doing so requires solving an optimal transport problem many times. To alleviate this problem, we upper bound the expected data matching term with an average cost. 




% $m \sim \sN(\mu,\Sigma)$ parameterized by $\mu\in\R^{ND}, \Sigma = I_D\otimes \diag{d}$,
...
% then diffeomorphic registration problem (\ref{eq:optimization_lddmm_landmark_momentum}) can be viewed as maximum a posteri problem, 

% Diffeomorphic registration can be formulated as maximum a posteriori problem,
