...
\begin{align*}
    I(\rc;\rx)
        ...
        &= \E_{\rc,\rz} \pb{ \log q_{\rc|\rx}(c|G(c,z)) } + H(\rc)
\end{align*}
...
\begin{align*}
    \nabla_{\theta} I(\rc;\rx) 
        &= \nabla_{\theta} \E_{\rc,\rx} \left[ \log q_{\rc|\rx}(c|x) \right] \\
...
Note $q_{\rc|\rx}$ is parameterized by a neural netowrk Q. Given the loss
\begin{align*}
    \sL_{GAN}(D,G) 
        &= \E_{x\sim p_{data}}\pb{\log D(x)} + \E_{z\sim p_{niose}} \pb{ \log(1-D(G(z))) } \\\
    \sL_{MI}(Q,G)
        &= \E_{\rc\sim \p{c},\rx\sim }
\end{align*}
